...
describe AppleEpf::Main do
  ...
  describe '#download_all_files' do
    before do
      files_matrix = { one: %w(test1 test2), two: %w(test3 test4) }
    ...
    it 'should send to block if block given' do
      ...
      end.to yield_successive_args(*[@downloder_double1, @downloder_double2])
    end
  end
  ...
  describe '#download_and_extract_all_files' do
    before do
      ...
      @file_matrix = {
        ...
        popularity: %w(popularity1 popularity2)
      }
      ...
      @popularity_expected_extracted = %w(popularity1 popularity2).map do |f|
    ...
    it 'should send to block if block given' do
      ...
      end.to yield_successive_args(AppleEpf::Extractor::FileEntry, AppleEpf::Extractor::FileEntry)
    end
  end
