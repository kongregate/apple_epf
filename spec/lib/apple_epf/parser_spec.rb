...
describe AppleEpf::Parser do
  let(:header_block) { proc { |b| } }
  let(:row_block) { proc { |b| } }
  ...
  describe 'perform' do
    describe '#parse_metadata' do
      it 'should properly parse header' do
        _header_info = { file: 'application',
                         columns: %w(export_date application_id title
    ...
    describe '#process_rows' do
      it 'should properly parse body' do
        ...
        end.to yield_successive_args(*[first_entry, second_entry, third_entry])
      end
      ...
      it 'should return nil for blank data' do
        ...
        expect do |b|
          parser.process_rows(&b)
        end.to yield_successive_args(*[first_entry])
      end
    end
  end
end
